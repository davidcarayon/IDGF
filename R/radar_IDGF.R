...
#' @export
#'
Radar_IDGF <- function(result_IDGF, lang = "FR"){
  ...
  for( i in id_releves) {
    ...
    ggplot2::ggplot(ggplot2::aes(x = param, y = radar_metric)) +
      ggplot2::geom_bar(stat = "identity", fill = "slateblue",color="black", alpha = 0.7) +
      ...
      ggplot2::theme(axis.text.x = ggplot2::element_text(face = "bold",size = 9))
